...
export default function CardFlip({
  ...
  useDefaultFrontStyle = false,
  children
}: CardFlipProps) {
  const [isFlipped, setIsFlipped] = useState(false);

  // Jika ada children, gunakan children untuk kedua sisi
  if (children) {
    return (
      <div
        style={{
  ...
  // Custom front content (untuk design seperti gambar Anda)
  const renderCustomFront = () => {
    if (frontContent) {
      return (
        <div className={cn(
          'absolute inset-0 h-full w-full',
    ...
    // Jika ada frontTitle, gunakan style minimalis seperti gambar
    if (frontTitle) {
      return (
        <div
          className={cn(
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center text-center space-y-4">
            ...
            {/* Title */}
            <h3 className="text-lg font-semibold tracking-tight text-white transition-all duration-500 group-hover:translate-y-[-2px]">
              {frontTitle}
            </h3>

            {/* Subtitle/Description */}
            {frontSubtitle && (
              <p className="text-sm text-zinc-400 max-w-[200px] transition-all duration-500 group-hover:translate-y-[-2px]">
                {frontSubtitle}
              </p>
            )}
            ...
            {/* Read More indicator */}
            <div className="flex items-center gap-2 text-xs text-primary transition-all duration-300 group-hover:gap-3 opacity-70 group-hover:opacity-100 mt-2">
    ...
    // Default style dengan animasi
    return null;
  };

  // Default front style
  const renderDefaultFront = () => (
    <div
      className={cn(
        'absolute inset-0 h-full w-full',
        '[transform:rotateY(0deg)] [backface-visibility:hidden]',
  ...
  return (
    ...
    >
      ...
      >
        {/* Front of card */}
        {renderCustomFront() || renderDefaultFront()}

        {/* Back of card */}
        {renderCustomBack() || renderDefaultBack()}
      </div>
